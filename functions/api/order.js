...
// Kortingscode — server-side de enige bron van waarheid.
    ...
    // 'Aan de deur betalen' is per direct alleen nog beschikbaar via de kassa
    // (functions/api/kassa-order.js, achter STAFF_LOYALTY_PASSWORD), NIET meer
    // via deze openbare bestelsite-endpoint. Reden (juli 2026, op verzoek van
    // Maikel): een 'aan_de_deur'-order werd altijd meteen als status='paid'
    // aangemaakt (= "klaar voor keuken/rit") zonder enige betaalcontrole
    // vooraf - alleen een belofte om aan de deur te betalen. Dat betekende dat
    // om het even wie, buiten bestellen.html om, dit endpoint rechtstreeks kon
    // aanroepen met betaalmethode: "aan_de_deur" en zo onbeperkt onbetaalde
    // 'spookbestellingen' kon aanmaken - dit is exact hoe testorder
    // BD-MS3F1JVG deze sessie is aangemaakt, ter demonstratie van het lek.
    // Verwijderen van de radio-knop in bestellen.html alleen was niet genoeg
    // (dat stopt alleen de UI, niet een rechtstreekse aanroep van dit
    // endpoint) - deze branch weigert daarom nu altijd, ongeacht wat er in de
    // request staat.
    if (betaalmethode === "aan_de_deur") {
      return json(
        {
          error:
            "'Aan de deur betalen' is niet meer beschikbaar via de website. Kies iDEAL, of bel ons voor een telefonische bestelling.",
        },
        400
      );
    }
